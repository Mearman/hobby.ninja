...
const SMALL_SQUARE_MAX_SIZE = 250; // Small square threshold for preview classification
	...
	try {
		...
		for (const filename of imageFiles) {
			...
			try {
				...
				results.errors.push(`Error processing ${filename}: ${message}`);
			}
		}

	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		results.errors.push(`Failed to read directory: ${message}`);
	}
...
function displayResults(results: ScanResults): void {
	...
	if (results.thumbnails.length > 0) {
		...
		console.log("----------------------------------------");
		for (const img of results.thumbnails) {
	...
	if (results.previews.length > 0) {
		...
		console.log("-------------------------------------");
		for (const img of results.previews) {
			console.log(`${img.itemId}_${img.imageNumber}: ${img.width}x${img.height} (${formatFileSize(img.size)})`);
		}
	}
 ...
 */
async function removeIncorrectImages(results: ScanResults, dryRun = true): Promise<void> {
