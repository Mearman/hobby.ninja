...
import { countTokens , EMBED_LINK_REGEX, readMarkdownFile } from "./markdown-utils";

const CLAUDE_FILE = "CLAUDE.md";
const TOKEN_THRESHOLD = 40_000;
const MAX_TOKEN_EXIT_CODE = 255;
...
// Build file tree
const buildFileTree = (filePath: string, visited = new Set<string>(), depth = 0): FileTreeNode | null => {
	...
	try {
		...
		for (const match of linkMatches) {
			...
			if (linkText === linkPath) {
				const child = buildFileTree(path.resolve(markdownFile.directory, linkPath), visited, depth + 1);
				if (child) children.push(child);
			}
		}

		return { filePath, relativePath: path.relative(process.cwd(), absolutePath), tokens, children, depth };
	} catch {
		return null;
	}
};
...
// Print tree
const printFileTree = (node: FileTreeNode | null, prefix = "", isLast = true): void => {
	if (!node) return;
	const connector = isLast ? "+-- " : "|-- ";
	const extension = node.children.length > 0 ? "" : (node.tokens > 1000 ? " [HIGH]" : node.tokens > 500 ? " [MED]" : " [LOW]");
...
const calculateTreeTokens = (node: FileTreeNode | null): number => {
	if (!node) return 0;
...
const mergedContent = readFileSync(CLAUDE_FILE, "utf8");
...
if (totalTokens > TOKEN_THRESHOLD) {
	...
	const exitCode = Math.min(totalTokens, MAX_TOKEN_EXIT_CODE);
	console.log(`\nExiting with code: ${exitCode.toString()} (token count)`);
