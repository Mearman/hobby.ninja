...
// Export schema definition types
...
export const FieldSchemaDefinition = z.object({
...
export const EnumSchemaDefinition = z.object({
	values: z.array(GraphValue),
	default: GraphValue.optional(),
});

export const ArraySchemaDefinition = z.object({
	itemsSchema: z.string(), // Reference to schema node ID
	minItems: z.number().optional(),
	maxItems: z.number().optional(),
});

export const RelationshipSchemaDefinition = z.object({
...
export class ArbitraryGraphManager {
	...
	getConnectedNodes(nodeId: string): DataNodeUnion[] {
		...
		const connectedIds = new Set(relationships.map(rel =>
			rel.fromNode === nodeId ? rel.toNode : rel.fromNode,
		));

		return this.getDataNodes().filter(node => connectedIds.has(node.id));
	}
