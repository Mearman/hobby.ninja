/* eslint-disable no-emoji/no-emoji, no-console, unicorn/no-process-exit, @typescript-eslint/no-unused-vars, @typescript-eslint/prefer-nullish-coalescing, @typescript-eslint/no-unnecessary-condition, no-useless-escape, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/restrict-template-expressions, unicorn/no-array-callback-reference, unicorn/no-array-sort */
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

import { transformEdgesToUltraCompact, type UltraCompactEdgeMap } from "./edge-transformer";
   ...
   */
    ...
    for (const manual of manualNodes) {
    	...
    	for (const seriesId of relatedSeriesIds) {
    		...
    		for (const id of items) relatedItemIds.add(id);
    	}
    ...
    for (const [scale, itemIds] of scaleItemsMap) {
    	const scaleId = scale.toLowerCase().replaceAll(/[\/\s:]+/g, "-");
    	const scaleData = {
    		id: scaleId,
    		type: "scale",
    		name: scale,
    ...
    staticParamsData.scaleIds = [...scaleItemsMap.keys()].map(scale =>
    	scale.toLowerCase().replaceAll(/[\/\s:]+/g, "-"),
    );
