"use client";

import type { Item } from "@hobby-ninja/data";
import { Breadcrumbs, Anchor, Group } from "@mantine/core";
import { IconHome } from "@tabler/icons-react";

import { itemConfig } from "@/components/lists/configs";
import { GenericListPage } from "@/components/lists/generic-list-page";

interface ScaleItemsClientProps {
	items: Item[];
	scaleName: string;
	totalItems: number;
}

export function ScaleItemsClient({ items, scaleName, totalItems }: ScaleItemsClientProps) {
	return (
		<GenericListPage
			items={items}
			totalItems={totalItems}
			config={itemConfig}
			pageTitle={`${scaleName} Items`}
			subtitle={`Browse ${totalItems.toLocaleString()} ${scaleName} items`}
			breadcrumbs={
				<Breadcrumbs>
					<Anchor href="/" size="sm">
						<Group gap={4}>
							<IconHome size={14} />
							Home
						</Group>
					</Anchor>
					<Anchor href="/database" size="sm">
						Database
					</Anchor>
					<Anchor href="/scales" size="sm">
						Scales
					</Anchor>
					<Anchor size="sm" fw={500}>
						{scaleName}
					</Anchor>
				</Breadcrumbs>
			}
		/>
	);
}
