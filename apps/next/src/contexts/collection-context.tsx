...
interface CollectionContextType {
  ...
  actions: {
    ...
    bulkAddItems: (items: Omit<CollectionItem, 'id' | 'dateAdded' | 'lastModified'>[]) => Promise<void>;
    bulkUpdateItems: (updates: { id: string; updates: Partial<CollectionItem> }[]) => Promise<void>;
...
export function CollectionProvider({ children, defaultCollectionId }: CollectionProviderProps) {
  ...
  const addItemAction = async (itemId: string, itemData: Omit<CollectionItem, 'id' | 'dateAdded' | 'lastModified'>) => {
    try {
      const item = await addItem(itemId, itemData);
  ...
  const bulkAddItemsAction = async (items: Omit<CollectionItem, 'id' | 'dateAdded' | 'lastModified'>[]) => {
    try {
  ...
  const bulkUpdateItemsAction = async (updates: { id: string; updates: Partial<CollectionItem> }[]) => {
    try {
