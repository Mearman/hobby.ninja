...
export class SearchIndex {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      items.forEach(item => {
        ...
        if (!validatedItem || !validatedItem.name) return;
      ...
      brands.forEach(brand => {
        ...
        if (!validatedBrand || !validatedBrand.name) return;
      ...
      categories.forEach(category => {
        ...
        if (!validatedCategory || !validatedCategory.name) return;
      ...
      seriesData.forEach(series => {
        ...
        if (!validatedSeries || !validatedSeries.name) return;
  ...
  getFilterOptions() {
    return {
      ...
      grades: Array.from(this.grades).sort(),
      scales: Array.from(this.scales).sort(),
      series: Array.from(this.series).sort()
    };
  }
   ...
   */
  getItemsByCategory(category: string, limit = 100): SearchResult[] {
   ...
   * Get items by brand
   */
  getItemsByBrand(brand: string, limit = 100): SearchResult[] {
   ...
   */
  getRelatedItems(itemId: string, limit = 10): SearchResult[] {
   ...
   */
  getRandomItems(count = 12, category?: string): SearchResult[] {
    let pool = category ?
