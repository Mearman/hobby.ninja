...
export const exportCollection = async (collectionId: string, includeHidden: boolean = false): Promise<string> => {
  try {
...
export const generateCollectionShareUrl = async (collectionId: string, includeHidden: boolean = false): Promise<string> => {
  try {
    ...
    const url = `${typeof window !== 'undefined' ? window.location.origin : ''}/collection/${collectionId}?share=${urlSafeBase64}`;
    return url;
  } catch (error) {
...
export const parseSharedCollection = async (shareData: string): Promise<{
  items: CollectionItem[];
  stats: {
    total: number;
    valid: number;
  ...
  try {
    ...
    return await importCollection(base64Data);
  } catch (error) {
...
export const exportCollectionCSV = async (collectionId: string, includeHidden: boolean = false): Promise<string> => {
  try {
    const base64Data = await exportCollection(collectionId, includeHidden);
    const compressed = Uint8Array.from(atob(base64Data), c => c.charCodeAt(0));
    const decompressed = pako.inflate(compressed);
