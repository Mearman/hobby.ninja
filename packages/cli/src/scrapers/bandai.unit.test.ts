...
describe("BandaiScraper", () => {
	...
	describe("fetchWithRetry", () => {
		...
		it("should cap backoff delay at maximum", async () => {
			...
			for (let i = 0; i < 5; i++) {
				await vi.advanceTimersByTime(10_000); // Max backoff
			}
	...
	describe("getCategories", () => {
		...
		it("should return default categories when not cached", async () => {
			...
			expect(mockCache.set).toHaveBeenCalledWith("categories", expect.any(Array), 86_400_000);
		});
	...
	describe("Edge cases and error handling", () => {
		...
		it("should handle memory pressure scenarios", async () => {
			const largeProduct: BandaiProduct = {
				sku: "LARGE-001",
				name: "Large Product " + "x".repeat(10_000),
				price: "¥1,000",
				category: "Test",
				grade: "HG",
				series: "Test Series",
				description: "x".repeat(50_000),
				specifications: { "large-field": "x".repeat(10_000) },
				urls: {},
				metadata: {
					scrapedAt: new Date().toISOString(),
					source: "bandai-official-store",
			...
			expect(result).toHaveLength(1);
			expect(result[0].name.length).toBeGreaterThan(10_000);
		});
