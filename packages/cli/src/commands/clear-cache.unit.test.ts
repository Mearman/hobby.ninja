...
describe("clearCacheCommand", () => {
	...
	describe("Edge cases and boundary conditions", () => {
		...
		it("should handle very large number of cache files", async () => {
			const manyFiles = Array.from({length: 10_000}).fill(null).map((_, i) => `cache${i}.json`);
			...
			expect(mockFs.unlink).toHaveBeenCalledTimes(10_000);
		...
		it("should handle concurrent cache clear operations", async () => {
			...
			const promises = Array.from({length: 5}).fill(null).map(() => clearCacheCommand(options));
