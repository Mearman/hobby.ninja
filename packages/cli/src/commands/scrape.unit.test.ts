...
describe("scrapeCommand", () => {
	...
	describe("Option parsing and validation", () => {
		...
		it("should handle invalid numeric options", async () => {
			...
			expect(BandaiScraper).toHaveBeenCalledWith(
				expect.objectContaining({
					timeout: Number.NaN, // parseInt('invalid') -> NaN
	...
	describe("Edge cases and boundary conditions", () => {
		...
		it("should handle extremely large data sets", async () => {
			const largeDataset = Array.from({length: 10_000}).fill(null).map((_, i) => ({
		...
		it("should handle very large timeout values", async () => {
			...
			expect(BandaiScraper).toHaveBeenCalledWith(
				expect.objectContaining({ timeout: 999_999_999 }),
			);
		});
	...
	describe("Performance and resource management", () => {
		it("should handle memory pressure scenarios", async () => {
			...
			const memoryIntensiveProducts = Array.from({length: 1000}).fill(null).map((_, i) => ({
				sku: `MEMORY-${i}`,
				name: "A".repeat(10_000), // 10KB name
				price: "¥1,000",
				category: "Test",
				grade: "HG",
				series: "Test Series",
				description: "B".repeat(50_000), // 50KB description
				urls: {},
				metadata: {
					scrapedAt: new Date().toISOString(),
					source: "test",
		...
		it("should handle rapid successive calls", async () => {
			const promises = Array.from({length: 5}).fill(null).map(() => scrapeCommand(mockOptions));
