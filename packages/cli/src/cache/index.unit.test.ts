...
describe('PageCache', () => {
  ...
  describe('Cache operations - basic get/set', () => {
    ...
    it('should handle custom TTL', async () => {
      ...
      await cache.set('custom-ttl-key', testData, 7200000); // 2 hours
  ...
  describe('Cache operations - cleanup', () => {
    ...
    it('should remove expired entries', async () => {
      ...
      const validEntry = {
        ...
        expiresAt: now + 1800000, // still valid
        size: 100,
      };
  ...
  describe('Cache size management', () => {
    ...
    it('should remove oldest entries during LRU eviction', async () => {
      ...
      mockFs.stat
        .mockResolvedValueOnce({ size: 500, ctime: new Date(now - 3600000) } as any)
  ...
  describe('Utility operations', () => {
    ...
    it('should extend TTL for existing keys', async () => {
      ...
      await cache.touch('test-key', 60000); // Extend to 1 minute

      const writeCall = mockFs.writeFile.mock.calls[0];
      const updatedEntry = JSON.parse(writeCall[1] as string);

      expect(updatedEntry.expiresAt).toBeGreaterThan(Date.now() + 50000);
    });
  });
  ...
  describe('Batch operations', () => {
    ...
    it('should perform multiple set operations', async () => {
      const entries = [
        ...
        { key: 'key3', data: 'value3', ttl: 7200000 },
      ];
