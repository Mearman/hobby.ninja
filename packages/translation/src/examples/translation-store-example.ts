/* eslint-disable no-console, @typescript-eslint/restrict-template-expressions, unicorn/prefer-module, unicorn/prefer-top-level-await, @typescript-eslint/no-unused-vars */
/**
		...
		switch (environment) {
			case "production": {
				...
				service = new TranslationService(
					{ cacheEnabled: true, timeout: 20_000 },
					undefined,
					serverStore,
				);
...
async function batchTranslationExample() {
	...
	try {
		...
		// Show some results
		for (const [index, result] of batchResult.results.slice(0, 3).entries()) {
