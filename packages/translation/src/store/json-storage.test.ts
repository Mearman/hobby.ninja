...
describe("JSONStorage", () => {
	...
	beforeEach(() => {
		...
		testDir = path.join(tmpdir(), `json-storage-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	describe("writeJSON", () => {
		...
		it("should ensure parent directory exists", async () => {
			const filePath = path.join(testDir, "nested", "path", "file.json");
	...
	describe("readJSON", () => {
		it("should read uncompressed JSON data", async () => {
			const filePath = path.join(testDir, "read-simple.json");
		...
		it("should read and decompress brotli data", async () => {
			const filePath = path.join(testDir, "read-brotli.json");
		...
		it("should read and decompress gzip data", async () => {
			const filePath = path.join(testDir, "read-gzip.json");
	...
	describe("deleteFile", () => {
		...
		it("should not throw when deleting non-existent file", async () => {
			const filePath = path.join(testDir, "delete-non-existent.json");
	...
	describe("ensureDirectory", () => {
		it("should create nested directories", async () => {
			...
			const testFile = path.join(dirPath, "test.json");
	...
	describe("integrity verification", () => {
		...
		it("should detect tampered files", async () => {
			const filePath = path.join(testDir, "tampered.json");
	...
	describe("statistics", () => {
		it("should track operation statistics", async () => {
			...
			await storage.writeJSON(path.join(testDir, "file1.json"), data);
			await storage.writeJSON(path.join(testDir, "file2.json"), largeData);
			await storage.readJSON(path.join(testDir, "file1.json"));
			await storage.readJSON(path.join(testDir, "file2.json"));
			await storage.deleteFile(path.join(testDir, "file1.json"));
		...
		it("should reset statistics", async () => {
			...
			await storage.writeJSON(path.join(testDir, "reset.json"), { test: true });
	...
	describe("concurrent access", () => {
		...
		it("should handle concurrent reads and writes", async () => {
			const filePath = path.join(testDir, "concurrent-rw.json");
	...
	describe("compression behavior", () => {
		it("should choose appropriate compression based on size", async () => {
			...
			const smallResult = await storage.writeJSON(path.join(testDir, "small.json"), smallData);
			expect(smallResult.compressionFormat).toBe("none");

			// Medium data - gzip compression
			const mediumData = { medium: "x".repeat(50) };
			const mediumResult = await storage.writeJSON(path.join(testDir, "medium.json"), mediumData);
			expect(mediumResult.compressionFormat).toBe("gzip");

			// Large data - brotli compression
			const largeData = { large: "x".repeat(200) };
			const largeResult = await storage.writeJSON(path.join(testDir, "large.json"), largeData);
		...
		it("should achieve better compression ratio for repetitive data", async () => {
			...
			const result = await storage.writeJSON(path.join(testDir, "repetitive.json"), repetitiveData);
	...
	describe("file format compatibility", () => {
		it("should reject files with invalid magic bytes", async () => {
			const filePath = path.join(testDir, "invalid-magic.json");
		...
		it("should reject files with corrupted metadata", async () => {
			const filePath = path.join(testDir, "corrupted-metadata.json");
