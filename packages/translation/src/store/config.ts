...
export const ConfigurationSchema = z.object({
  ...
  maxEntries: z.number()
    ...
    .max(100000, 'Maximum entries cannot exceed 100,000'),
  ...
  lockTimeout: z.number()
    ...
    .max(60000, 'Lock timeout cannot exceed 60 seconds'),
  ...
  cacheSize: z.number()
    ...
    .max(10000, 'Cache size cannot exceed 10,000'),
...
export const DEFAULT_CONFIG: StoreConfiguration = {
  ...
  storageDir: join(homedir(), '.gundam-cache', 'translations'),
 ...
 */
function parseEnvValue(key: keyof typeof ENV_VAR_MAPPINGS, value: string): any {
  switch (key) {
    case 'storageDir':
      return value.trim();
...
export function resolveConfigPath(configPath?: string): string {
  ...
  const possiblePaths = [
    join(process.cwd(), 'translation.config.json'),
    join(process.cwd(), '.translationrc'),
    join(homedir(), '.gundam', 'translation.config.json'),
  ];
  ...
  return join(process.cwd(), 'translation.config.json');
}
