/* eslint-disable no-console, @typescript-eslint/restrict-template-expressions, sonarjs/no-duplicate-string */
/**
...
export async function basicStorageExample(): Promise<void> {
	...
	try {
		...
		const testFile = path.join(tmpdir(), `gundma-example-${Date.now()}.json`);
...
export async function concurrentAccessExample(): Promise<void> {
	...
	const storage = new JSONStorage();
	const testFile = path.join(tmpdir(), `concurrent-example-${Date.now()}.json`);
...
export async function errorHandlingExample(): Promise<void> {
	...
	const storage = new JSONStorage();
	const testFile = path.join(tmpdir(), `error-example-${Date.now()}.json`);
