...
export class Downloader {
  ...
  constructor(outputDir: string = 'data/bandai/manuals') {
   ...
   */
  private migrateOldIndexToNewFormat(oldIndex: { [id: string]: IndexEntry }): CompactManualIndex {
   ...
   */
  private async recordIdCheck(id: number, isValid: boolean, hasFile: boolean = false): Promise<void> {
  ...
  private addInvalidId(id: number): void {
    ...
    let merged = this.tryMergeWithRanges(id);
  ...
  async download(options: DownloaderOptions = {}) {
    ...
    try {
      if (existingFilesInRange.size > 0) {
        ...
        const { boundaries: expansionSeeds, ranges } = this.getBoundarySeeds(Array.from(existingFiles));
  ...
  private generateCoverageReport(startId: number, endId: number): void {
    ...
    while (currentId <= endId) {
      const isCovered = validIdsInRange.indexOf(currentId) !== -1 ||
        invalidRangesInRange.some(range => currentId >= range.start && currentId <= range.end) ||
        invalidSinglesInRange.indexOf(currentId) !== -1;

      if (!isCovered) {
        const gapStart = currentId;
        while (currentId <= endId && validIdsInRange.indexOf(currentId) === -1 &&
          !invalidRangesInRange.some(range => currentId >= range.start && currentId <= range.end) &&
          invalidSinglesInRange.indexOf(currentId) === -1) {
          currentId++;
        }
  ...
  private async exploreUnexploredTerritories(baseUrl: string, startId: number, endId: number, existingFiles: Set<number>): Promise<number[]> {
    ...
    for (let chunkStart = startId; chunkStart <= endId; chunkStart += chunkSize) {
      ...
      const chunkValidIds = Array.from(existingFiles).filter(id => id >= chunkStart && id <= chunkEnd);
