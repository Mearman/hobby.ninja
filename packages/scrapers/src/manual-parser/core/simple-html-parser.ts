...
export class SimpleHtmlParser {
  ...
  private extractTitle(document: any): string | undefined {
    ...
    if (titleElement && titleElement.childNodes) {
  ...
  private extractContent(document: any): any[] {
    ...
    return blocks.filter(block => block && block.content && Object.keys(block.content).length > 0);
  }
  ...
  private processNode(node: any, blocks: any[]): void {
    ...
    if (node.nodeName === '#text') {
      ...
      if (text.trim()) {
        if (this.hasJapaneseContent(text)) {
          blocks.push({
            type: 'text',
            content: {
              text: text.trim(),
